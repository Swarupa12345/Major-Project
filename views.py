...
def classification_report_csv(report):
    ...
    dataframe.to_csv('svm_report.csv', index = False)
...
def svm(request):
    ...
    data = data[['AGE','PTEDUCAT','MMSE','ADAS11','ADAS13','D2']]
    ...
    print("M values:",m)
    print("Accuracy Values:",cm)
    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred)
    recall = recall_score(y_test, y_pred)
    f1Score = f1_score(y_test, y_pred)
    rsltdict = {'accuracy':accuracy,'precision':precision,'recall':recall,'f1Score':f1Score}
    #x = accurancy.split()

    #classification_report_csv(accurancy)
    #c_matrix = pd.read_csv('svm_report.csv')
    ...
    return render(request, 'admin/svmresults.html', rsltdict)
...
def adminDecisionTree(request):
    ...
    return render(request, 'admin/dtresults.html', rsltdict)
